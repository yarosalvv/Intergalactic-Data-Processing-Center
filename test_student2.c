...
void test_bubble_sort() {
    ...
    printf("Array of 1 element: %.1f - %s\n", arr1[0], 
           double_equals(arr1[0], 5.0, 0.001) ? "PASS" : "FAIL");
    ...
    int sorted3 = 1;
    for (int i = 0; i < 4; i++) {
        if (arr3[i] > arr3[i+1]) sorted3 = 0;
    }
    printf("Array of 5 elements: %.1f,%.1f,%.1f,%.1f,%.1f - %s\n",
...
void test_statistical_functions() {
    ...
    printf("  Mean: %.1f (expected 3.0) - %s\n", mean1,
           double_equals(mean1, 3.0, 0.001) ? "PASS" : "FAIL");
    printf("  Median: %.1f (expected 3.0) - %s\n", median1,
           double_equals(median1, 3.0, 0.001) ? "PASS" : "FAIL");
    printf("  Min: %.1f (expected 1.0) - %s\n", min,
           double_equals(min, 1.0, 0.001) ? "PASS" : "FAIL");
    printf("  Max: %.1f (expected 5.0) - %s\n", max,
           double_equals(max, 5.0, 0.001) ? "PASS" : "FAIL");
    ...
    printf("Set [1,2,3,4]:\n");
    printf("  Mean: %.1f (expected 2.5) - %s\n", mean2,
           double_equals(mean2, 2.5, 0.001) ? "PASS" : "FAIL");
    printf("  Median: %.1f (expected 2.5) - %s\n", median2,
           double_equals(median2, 2.5, 0.001) ? "PASS" : "FAIL");
    printf("  Min: %.1f (expected 1.0) - %s\n", min,
           double_equals(min, 1.0, 0.001) ? "PASS" : "FAIL");
    printf("  Max: %.1f (expected 4.0) - %s\n", max,
           double_equals(max, 4.0, 0.001) ? "PASS" : "FAIL");
    ...
    printf("  Mean: %.1f (expected 0.0) - %s\n", mean3,
           double_equals(mean3, 0.0, 0.001) ? "PASS" : "FAIL");
    printf("  Median: %.1f (expected 0.0) - %s\n", median3,
           double_equals(median3, 0.0, 0.001) ? "PASS" : "FAIL");
    printf("  Min: %.1f (expected -2.0) - %s\n", min,
           double_equals(min, -2.0, 0.001) ? "PASS" : "FAIL");
    printf("  Max: %.1f (expected 2.0) - %s\n", max,
           double_equals(max, 2.0, 0.001) ? "PASS" : "FAIL");
...
void test_invalid_records() {
    ...
    printf("All valid records: count=%d - %s\n", count1,
           (count1 == 3) ? "PASS" : "FAIL");
    
    // One invalid record
    int count2 = convert_numbers(invalid_strings, invalid_bases, 3, results);
    printf("One invalid record: count=%d - %s\n", count2,
           (count2 == -1) ? "PASS" : "FAIL");
